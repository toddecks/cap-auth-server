...
const FANTASY_FOOTBALL_BLAST_RUN_ID = "B06EBAFA-C839-47DD-AFF5-C535CEDB7DE4";
let fantasyFootballBlastInProgress = false;
let fantasyFootballBlastResults = null;
...
app.post("/api/fantasy-football-invite-blast", async (req, res) => {
  res.set("Cache-Control", "no-store");
  if (req.body?.campaignRunId !== FANTASY_FOOTBALL_BLAST_RUN_ID) {
    return res.status(403).json({ error: "This fantasy campaign run is not authorized." });
  }
  if (fantasyFootballBlastResults) {
    return res.json({ ok: true, repeated: true, ...fantasyFootballBlastResults });
  }
  if (fantasyFootballBlastInProgress) {
    return res.status(409).json({ error: "The fantasy campaign is already sending." });
  }
  ...
  if (recipients.length !== 16 || uniqueEmails.size !== 16 || blueCount !== 8 || goldCount !== 8 || !validRecipients) {
    return res.status(400).json({ error: "The campaign must contain eight unique Blue and eight unique Gold recipients." });
  }
  if (!RESEND_API_KEY || !PRO_FORMS_FROM_EMAIL) {
    return res.status(503).json({ error: "Fantasy football email delivery is not configured." });
  }

  fantasyFootballBlastInProgress = true;
  ...
  try {
    for (const recipient of recipients) {
      ...
      try {
        ...
        const providerId = await sendFantasyFootballEmail({
          ...
          subject: `Your ${config.leagueName} Fantasy Football Invite`,
          html,
          attachments: [{
          ...
          tags: [
            ...
            { name: "delivery", value: "blast" }
          ],
          idempotencyKey: `fantasy-2026-${recipient.league}-${recipientKey}`
        });
    ...
    fantasyFootballBlastResults = {
      sentAt: new Date().toISOString(),
      accepted: deliveries.filter((entry) => entry.accepted).length,
      failed: deliveries.filter((entry) => !entry.accepted).length,
      deliveries
    };
    return res.status(fantasyFootballBlastResults.failed ? 207 : 200).json({
      ok: fantasyFootballBlastResults.failed === 0,
      ...fantasyFootballBlastResults
    });
  } finally {
    fantasyFootballBlastInProgress = false;
  }
});

app.get("/api/fantasy-football-invite-delivery", async (req, res) => {
  res.set("Cache-Control", "no-store");
  if (req.query?.campaignRunId !== FANTASY_FOOTBALL_BLAST_RUN_ID) {
    return res.status(403).json({ error: "This fantasy campaign run is not authorized." });
  }
  if (!fantasyFootballBlastResults) {
    return res.status(404).json({ error: "The fantasy campaign has not been sent from this server instance." });
  }
  ...
  try {
    ...
    const deliveries = fantasyFootballBlastResults.deliveries.map((entry) => ({
      ...entry,
      lastEvent: entry.providerId ? historyById.get(entry.providerId)?.last_event || "pending" : "failed"
    }));
    return res.json({
      ok: true,
      sentAt: fantasyFootballBlastResults.sentAt,
      deliveries
    });
  } catch (error) {
    console.error("Fantasy football delivery lookup failed:", error?.message || error);
